...
public class NotificationController {
    ...
    @PostMapping("/AddNoti")
    public ResponseEntity<?> addNoti(@RequestBody NotificationVo notiVo, PostnotiVo postnotiVo, Authentication authentication) {
        try {
	...
	public ModelAndView getNotificationFragment(ProfileVo profile, Authentication authentication, @AuthenticationPrincipal OAuth2User oAuth2User) {
    ...
    public NotificationVo sendNotification(NotificationVo notification) {
        // 비즈니스 로직 수행 후 알림 객체를 반환합니다.
        return notification;
    }
    
    @ResponseBody
    @GetMapping("/notify")
    public ResponseEntity<Void> handleNotification(@RequestParam("notificationIdx") Long notificationIdx, @RequestParam("to_id") String to_id) {
